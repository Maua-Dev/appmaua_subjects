...
class Test_GetCountStudentsByScoreUsecase:
    ...
    async def test_get_count_student_by_score_1(self):
        ...
        count = await getCountStudentsByScoreUsecase(9.5, 'ECM505', 1, 2022, 1, 2)
        assert count == 2

    @pytest.mark.asyncio
    async def test_get_count_student_by_score_2(self):
        getCountStudentsByScoreUsecase = GetCountStudentsByScoreUsecase(subjectRepository=SubjectRepositoryMock())
        count = await getCountStudentsByScoreUsecase(9.5, 'EcM505', 2, 2022, 1, 2)
        assert count == 0

    @pytest.mark.asyncio
    async def test_get_count_student_by_score_3(self):
        getCountStudentsByScoreUsecase = GetCountStudentsByScoreUsecase(subjectRepository=SubjectRepositoryMock())
        count = await getCountStudentsByScoreUsecase(8.5, 'EcM505', 8, 2022, 1, 2)
        assert count == 1
    ...
    async def test_get_count_student_by_score_error_1(self):
        ...
        with pytest.raises(UnexpectedError):
            await getCountStudentsByScoreUsecase(9.5, 'a505', 2, 2022, 1, 2)

    @pytest.mark.asyncio
    async def test_get_count_student_by_score_error_2(self):
        getCountStudentsByScoreUsecase = GetCountStudentsByScoreUsecase(subjectRepository=SubjectRepositoryMock())
        with pytest.raises(UnexpectedError):
            await getCountStudentsByScoreUsecase(-1.5, 'ecm505', 2, 2022, 1, 2)

    @pytest.mark.asyncio
    async def test_get_count_student_by_score_error_3(self):
        getCountStudentsByScoreUsecase = GetCountStudentsByScoreUsecase(subjectRepository=SubjectRepositoryMock())
        with pytest.raises(UnexpectedError):
            await getCountStudentsByScoreUsecase(9.5, 'ecm505', 50, 2022, 1, 2)

    @pytest.mark.asyncio
    async def test_get_count_student_by_score_error_4(self):
        getCountStudentsByScoreUsecase = GetCountStudentsByScoreUsecase(subjectRepository=SubjectRepositoryMock())
        with pytest.raises(UnexpectedError):
            await getCountStudentsByScoreUsecase(9.5, 'ecm505', 2, None, 1, 2)
